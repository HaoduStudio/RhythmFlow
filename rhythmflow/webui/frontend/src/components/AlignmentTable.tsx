  ...
  const columns: ColumnsType<RowState> = [
    {
      ...
      render: (name: string, row) => (
        <Tooltip title={row.warnings.length ? row.warnings.join(", ") : row.video_path}>
          {name}
        </Tooltip>
      ),
    },
    {
      ...
      render: (_v, row) =>
        row.error
          ? t(lang, "error")
          : row.detected_offset == null
            ? ""
            : row.detected_offset.toFixed(3),
    },
    {
    ...
    },
    {
      ...
      align: "right",
      render: (_v, row) =>
        row.analyzed && !row.error
          ? `${row.smart_trim_s.toFixed(2)}s / ${row.smart_trim_count}`
          : "",
    },
    {
      ...
      render: (_v, row) =>
        !row.analyzed || row.error
          ? ""
          : row.smart_confidence == null
            ? "-"
            : row.smart_confidence.toFixed(2),
    },
    {
