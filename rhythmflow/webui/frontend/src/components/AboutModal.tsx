...
export function AboutModal(): JSX.Element {
  ...
  useEffect(() => {
    if (store.aboutOpen && !info) {
      void getApi().then((api) => api.about_info().then(setInfo));
    }
    ...
    () =>
      onEvent("update_status", (payload: UpdateStatusPayload) => {
        ...
        if (payload.status === "up_to_date") {
          ...
          message.success(
            t(lang, "update_restart_pending", { version: payload.latest_version ?? "" }),
          );
  ...
  const checking = updateStatus
    ? ["checking", "downloading", "installing", "restart_pending"].includes(updateStatus.status)
    : false;
